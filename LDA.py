...
class LDA:
    def __init__(self, K, alpha, beta, docs, V, smartinit=True):
        ...
        self.docs = docs #array of arrays of numbers, where each inner array represents a document, and its contents are word IDs
        ...
        self.N = 0 #total number of words in all documents
        for m, doc in enumerate(docs):
            
            self.N += len(doc)
            #z_n[i, j] means word j in document i is assigned to topic z_n[i,j]
            z_n = []
            for t in doc:
                if smartinit:
    ...
    def inference(self):
        ...
        for m, doc in enumerate(self.docs):
            z_n = self.z_m_n[m] #1-D array for topic number of each word in document m
            n_m_z = self.n_m_z[m] #1-D array for number of words for each topic in document m
            for n, t in enumerate(doc):
                # discount for n-th word t with topic z
                z = z_n[n] #z is the topic that word n is assigned
                n_m_z[z] -= 1
                self.n_z_t[z, t] -= 1
                self.n_z[z] -= 1
    ...
    def worddist(self):
        ...
        #print(self.n_z_t)
        #print(self.n_z[:,numpy.newaxis])

        return self.n_z_t / self.n_z[:, numpy.newaxis]

    def perplexity(self, docs=None):
        if docs == None: docs = self.docs
        phi = self.worddist() #K x V array, sum of each row is 1
        #print(sum(phi[1,:]))
    ...
    for i in range(iteration):
        ...
        if pre_perp:
            if pre_perp < perp:
                ...
                pre_perp = None #once set to None, there will be no more iterations
            else:
                pre_perp = perp
    output_word_topic_dist(lda, voca)
...
def main():
    ...
    #print(numpy.random.randint(0, options.K))
    if options.filename:
